...
const {
    createSettlement,
} = require("../Partner-Settlement/partner-settlement");
...
const createEarningInternal = async (driverId, orderId) => {
    ...
    if (existingEarning) {
        console.log(
            `⚠️ Earning already exists for order ${orderId} and driver ${driverId}`,
        );
    ...
    if (order.orderStatus !== 3) {
        throw new ApiError(
            400,
            `Order status must be 3 (Delivered). Current status: ${order.orderStatus}`,
        );
    }
    ...
    const deliveryNumber =
        lastEarning && lastEarning.deliveryNumber
    ...
    if (!settlement) {
        console.warn(
            `⚠️ Partner settlement not found for order ${orderId}, creating it now`,
        );
...
exports.getSettlementSummary = asyncHandler(async (req, res) => {
    ...
    const totalSettlements = await DriverSettlement.countDocuments({
        driverId,
    });
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                formattedEarnings,
                "Recent earnings retrieved successfully",
            ),
        );
});
